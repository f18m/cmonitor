...
class GoogleChartsTimeSeries(object):
    ...
    def toGoogleChartTable(self, graphName):
        ...
        if SAVE_DEFLATED_JS_DATATABLES:
            ...
            ret_string += (
                "var deflated_data_binary_%s = window.atob(deflated_data_base64_%s);\n"
                % (graphName, graphName)
            )

            # now inflate it in the browser using "pako" library (https://github.com/nodeca/pako)
            ret_string += (
                "var inflated_data_%s = JSON.parse(pako.inflate(deflated_data_binary_%s, { to: 'string' }));\n"
                % (graphName, graphName)
            )
        else:
        ...
        ret_string += (
            "var data_%s = google.visualization.arrayToDataTable(inflated_data_%s);\n"
            % (graphName, graphName)
        )
        return ret_string
...
class GoogleChartsGraph:
    def __init__(
        ...
        self.source_data = (
            graph_source  # one of GRAPH_TYPE_BAREMETAL or GRAPH_TYPE_CGROUP
        )
    ...
    def genGoogleChartJS_AreaChart(self):
        ...
        if len(self.series_for_2nd_yaxis) > 0:
            ...
            series_for_1st_yaxis = [
                item
                for item in series_for_1st_yaxis
                if item not in self.series_for_2nd_yaxis
            ]
            ...
            ret_string += (
                '  vAxis: { title: "%s", gridlines: { color: "lightgrey", count: 11 } },\n'
...
"""
    ...
    def endHtmlHead(self, config):
        ...
        for num, graph in enumerate(self.graphs, start=1):
            if len(graph.combobox_label) == 0:
                self.file.write(
                    '  document.getElementById("btn_draw_%s").addEventListener("click", draw_%s);\n'
...
"""
        ...
        for num, comboname in enumerate(sorted(combo_box_ctrls), start=1):
            self.file.write(
                '  if (combobox_to_exclude_from_reset != "%s")\n' % comboname
            )
            self.file.write(
                '      document.getElementById("select_combobox_%s").value = "clear_chart";\n'
                % comboname
            )
    ...
    def startHtmlBody(self, cgroupName, monitored_system):
        ...
        self.file.write(
            '  <h1>Monitoring data collected from <span id="monitored_system_span">'
            + monitored_system
            + "</span></h1>\n"
        )
        ...
        self.file.write("  <tr>\n")
        self.file.write(
            '    <td id="button_table_col"></td><td id="button_table_col"><b>CGroup</b> (Data collected from %s)</td>\n'
            % cgroupName
        )
        self.file.write(
        ...
        def write_buttons_for_graph_type(source_data):
            ...
            for num, graph in enumerate(self.graphs, start=1):
                if graph.source_data == source_data and len(graph.combobox_label) > 0:
                    ...
                    graphs_combobox[graph.combobox_label].append(
                        [graph.combobox_entry, graph.js_name]
                    )
            ...
            if len(graphs_combobox) > 0:
                for combobox_label in graphs_combobox.keys():
                    ...
                    self.file.write(
                        '    <select id="select_combobox_%s" onchange="call_function_named(this.value)">\n'
                        % (combobox_label)
                    )
...
def print_data_loading_stats(desc, jdata, n_invalid_samples):
    if n_invalid_samples > 0:
        print(
            "While parsing %s statistics found %d/%d (%.1f%%) samples that did not contain the JSON section."
            % (
                desc,
                n_invalid_samples,
                len(jdata),
...
def generate_config_js(jheader):
    ...
    if "cgroup_config" in jheader:
        ...
        jheader["cgroup_config"]["memory_limit_bytes"] = sizeof_fmt(
            int(jheader["cgroup_config"]["memory_limit_bytes"])
        )
    ...
    if "proc_meminfo" in jheader:
        jheader["proc_meminfo"]["MemTotal"] = sizeof_fmt(
            int(jheader["proc_meminfo"]["MemTotal"])
        )
        jheader["proc_meminfo"]["Hugepagesize"] = sizeof_fmt(
            int(jheader["proc_meminfo"]["Hugepagesize"])
        )
    ...
    if (
        "cgroup_config" in jheader
    ):  # if cgroups are off, this section will not be present
        config_str += configdump(
            "cgroup_config", "Linux Control Group (CGroup) Configuration"
        )
...
def generate_cgroup_topN_procs(web, header, jdata, numProcsToShow=20):
    ...
    for sample in jdata:
        try:
            for process in sample["cgroup_tasks"]:
                ...
                membytes = entry[
                    "mem_rss_bytes"
                ]  # take RSS, more realistic/useful compared to the "mem_virtual_bytes"
                thepid = entry["pid"]
    ...
    topN_process_table = GoogleChartsGenericTable(
        ["Command", "CPU time", "I/O " + io_unit, "Command", "Memory " + mem_unit]
    )
    for i, pid in enumerate(topN_process_pids_list):
        ...
        if verbose:
            print(
                "Processing data for %d-th CPU-top-scorer process [%s]"
                % (i + 1, nicecmd)
            )
        topN_process_table.addRow(
            [p["cmd"], p["cpu"], p["io"] / io_divider, nicecmd, p["mem"] / mem_divider]
        )

    # generate the bubble chart graph:
    web.appendGoogleChart(
        GoogleChartsGraph(
            button_label="CPU/Memory/Disk Bubbles",
            graph_source=GRAPH_SOURCE_DATA_CGROUP,
            graph_type=GRAPH_TYPE_BUBBLE_CHART,
            graph_title="Top %d processes/threads for CPU/disk total usage on X/Y axes; memory usage as bubble size (from cgroup stats)"
            % numProcsToShow,
    ...
    for key in ["cpu", "io", "mem"]:
        process_table[key] = GoogleChartsTimeSeries(
            ["Timestamp"] + [get_nice_cmd(pid) for pid in topN_process_pids_list]
        )
    for sample in jdata:
        try:
            row = {}
            ...
            for top_process_pid in topN_process_pids_list:
                ...
                if json_key in sample["cgroup_tasks"]:
                    ...
                    row["io"].append(
                        top_proc_sample["io_rchar"] + top_proc_sample["io_wchar"]
                    )
    ...
    web.appendGoogleChart(
        GoogleChartsGraph(
            ...
            graph_title="Top %d processes/threads for CPU usage (from cgroup stats)"
            % numProcsToShow,
    ...
    web.appendGoogleChart(
        GoogleChartsGraph(
            ...
            graph_title="Top %d processes/threads for I/O usage (from cgroup stats)"
            % numProcsToShow,
    ...
    web.appendGoogleChart(
        GoogleChartsGraph(
            ...
            graph_title="Top %d processes/threads for memory usage (from cgroup stats)"
            % numProcsToShow,
...
def generate_baremetal_cpus(web, jdata, logical_cpus_indexes):
    ...
    all_cpus_table = GoogleChartsTimeSeries(
        ["Timestamp"] + [("CPU" + str(x)) for x in logical_cpus_indexes]
    )
        ...
        try:
            ...
            if "throttling" in s["cgroup_cpuacct_stats"]:
                ...
                if nr_periods:
                    cpu_throttling = (
                        100
                        * s["cgroup_cpuacct_stats"]["throttling"]["nr_throttled"]
                        / nr_periods
                    )
...
def generate_baremetal_memory(web, jdata):
    ...
    baremetal_memory_stats = GoogleChartsTimeSeries(
        ["Timestamp", "Used", "Cached (DiskRead)", "Free"]
    )
...
def generate_cgroup_memory(web, jheader, jdata):
    ...
    if "memory_limit_bytes" in jheader["cgroup_config"]:
        ...
        mem_total_bytes = (
            jdata[0]["cgroup_memory_stats"]["total_cache"]
            + jdata[0]["cgroup_memory_stats"]["total_rss"]
        )
    cgroup_memory_stats = GoogleChartsTimeSeries(
        ["Timestamp", "Used", "Cached (DiskRead)", "Alloc Failures", "Limit"]
    )
    divider, unit = choose_byte_divider(mem_total_bytes)
...
def generate_baremetal_avg_load(web, jheader, jdata):
    ...
    load_avg_stats = GoogleChartsTimeSeries(
        ["Timestamp", "LoadAvg (1min)", "LoadAvg (5min)", "LoadAvg (15min)"]
    )
    for i, s in enumerate(jdata):
        if i == 0:
            continue  # skip first sample
...
def generate_monitoring_summary(jheader, jdata):
    ...
    monitoring_summary = [
        ...
        ("Collected:", jheader["cmonitor"]["collecting"].replace(",", ", ")),
        ...
        (
            ...
            str(
                datetime.timedelta(
                    seconds=jheader["cmonitor"]["sample_interval_seconds"] * len(jdata)
                )
            ),
        ),
        ("Version (cmonitor_collector):", jheader["cmonitor"]["version"]),
        ("Version (cmonitor_chart):", CMONITOR_VERSION),
    ]
...
def generate_about_this(jheader, jdata):
    about_this = [
...
def main_process_file(infile, outfile):
    ...
    if "stat" in jdata[0]:
        baremetal_logical_cpus_indexes = collect_logical_cpu_indexes_from_section(
            jdata_first_sample, "stat"
        )
        if verbose:
            print(
                "Found %d CPUs in baremetal stats with logical indexes [%s]"
                % (
                    len(baremetal_logical_cpus_indexes),
                    ", ".join(str(x) for x in baremetal_logical_cpus_indexes),
                )
            )

    cgroup_logical_cpus_indexes = []
    if "cgroup_cpuacct_stats" in jdata_first_sample:
        cgroup_logical_cpus_indexes = collect_logical_cpu_indexes_from_section(
            jdata_first_sample, "cgroup_cpuacct_stats"
        )
        if verbose:
            print(
                "Found %d CPUs in cgroup stats with logical indexes [%s]"
                % (
                    len(cgroup_logical_cpus_indexes),
                    ", ".join(str(x) for x in cgroup_logical_cpus_indexes),
                )
            )
    ...
    web.appendHtmlTable(
        "Monitoring Summary", generate_monitoring_summary(jheader, jdata)
    )
    if len(baremetal_logical_cpus_indexes) > 0:
        web.appendHtmlTable(
            "Monitored System Summary",
            generate_monitored_summary(jheader, jdata, baremetal_logical_cpus_indexes),
        )
    elif len(cgroup_logical_cpus_indexes) > 0:
        web.appendHtmlTable(
            "Monitored System Summary",
            generate_monitored_summary(jheader, jdata, cgroup_logical_cpus_indexes),
        )
    ...
    end_time = time.time()
    print(
        "Completed processing of input JSON file of %d samples in %.3fsec. HTML output file is ready."
        % (len(jdata), end_time - start_time)
    )
...
def usage():
    ...
    print(
        "  -u, --utc                  Plot data using UTC timestamps instead of local timezone"
    )
    sys.exit(0)
...
def parse_command_line():
    ...
    for o, a in opts:
        if o in ("-i", "--input"):
            ...
            g_datetime_field = (
                "UTC"  # instead of default 'datetime' which means local timezone
            )
        elif o in ("--version"):
    ...
    return {"input_json": input_json, "output_html": output_html}
